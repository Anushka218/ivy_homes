...
import { Home, Bookmark, LogOut, User } from 'lucide-react';
...
export function Navbar() {
  ...
  const isListingsActive = location.pathname === '/' || location.pathname.startsWith('/listings');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-100 text-xs text-slate-700">
            ...
            <span className="font-medium truncate max-w-[180px]">{user?.email || 'User'}</span>
          </div>
